...
"""Investigates the use of a general metric as a distinguishing
factor.

The metric is specifd by the currentMetric module variable.

This script was used to generate the figure in the Percentage Of Links
As A Distinguishing Factor on the Model Profiling page. The parameters
were -m 1 -M 100 -s 1 -n 20 -r 1000 completeOntology.json
ontology.json. The currentMetric was set to
linkograph.stats.percentageOfLinks.
...
currentMetric = lstats.percentageOfLinks
metricName = 'Percentage of Links'


def genSingleOntologyStats(metric, ontNext, ontLink, minLinkoSize,
...
if __name__ == '__main__':
    ...
    results = genSingleOntologyStats(metric=currentMetric,
                                     ontNext=ontNext,
                                     ontLink=ontLink,
    ...
    for metricsValues in results:
        plt.plot(linkoSizes, metricsValues)

    plt.xlabel('Linkograph sizes')
    plt.ylabel(metricName)
    plt.title(metricName + ' For Random Markov Models')
