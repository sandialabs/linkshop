...
not generated using an ontology.

This script was used to generate the Randomly Generated Models section
with -m 1 -M 100 -r 1000 -s 1 ontology.json. The ontology.json is the
v2_ontology.
...
def genSingleOntologyStats(ontology, minLinkoSize, maxLinkoSize,
    ...
    for i in range(runNum):
        m = markel.genModel(ontSize, absClasses=absClasses,
                            ontology=ontology,
                            seed=seeds[i])
...
if __name__ == '__main__':
    ...
    results = genSingleOntologyStats(ontology=ont,
    ...
    exceedLimit = len(ont)**2 > args.graphLimit

    linkoSizes = range(args.minimum, args.maximum, args.step)

    for row in range(len(ont)):
        for col in range(len(ont)):
            plt.figure(row*len(ont) + col)
            ...
            exceedGroupSize = (row*len(ont) + col) % groupSize == (groupSize-1)
